...
import {
  listGlobalFoodsAction,
  getGlobalFoodDetailsAction,
  createGlobalFoodAction,
  updateGlobalFoodAction,
  archiveGlobalFoodAction,
  createGlobalPortionAction,
  archiveGlobalPortionAction,
...
interface GlobalFoodManagerProps {
  initialResult: ListFoodsResult;
}

export function GlobalFoodManager({ initialResult }: GlobalFoodManagerProps) {
  ...
  function fetchFoods(targetPage = 1, currentQuery = query, currentStatus = statusFilter) {
    startTransition(async () => {
      const res = await listGlobalFoodsAction({
        query: currentQuery,
      ...
      if (res.success && res.data) {
        ...
        showFeedback("error", res.error || "Erro ao buscar alimentos globais.");
      }
    });
  }
  ...
  async function handleOpenDetails(food: FoodListItemDto) {
    ...
    try {
      const res = await getGlobalFoodDetailsAction(food.publicId);
      if (res.success && res.data) {
  ...
  async function handleCreateFood(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await createGlobalFoodAction(input);
      if (res.success) {
        showFeedback("success", "Alimento global cadastrado com sucesso!");
        setIsCreateOpen(false);
        fetchFoods(1);
      } else {
        showFeedback("error", res.error || "Erro ao cadastrar alimento global.");
      }
    });
  }
  ...
  async function handleUpdateFood(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await updateGlobalFoodAction(editingFood.publicId, input);
      if (res.success) {
  ...
  async function handleArchiveFood(food: FoodListItemDto) {
    if (!confirm(`Deseja arquivar o alimento global "${food.name}"?`)) return;

    startTransition(async () => {
      const res = await archiveGlobalFoodAction(food.publicId);
      if (res.success) {
        showFeedback("success", "Alimento arquivado!");
        fetchFoods(data.page);
      } else {
  ...
  async function handleAddPortion(e: React.FormEvent) {
    ...
    if (!label || isNaN(amount) || amount <= 0) {
      showFeedback("error", "Informe rótulo e quantidade válida.");
      return;
    }

    setIsAddingPortion(true);
    try {
      const res = await createGlobalPortionAction(selectedFood.publicId, {
        label,
      ...
      if (res.success) {
        ...
        setNewPortionLabel("");
        setNewPortionAmount("");
        const updated = await getGlobalFoodDetailsAction(selectedFood.publicId);
        if (updated.success && updated.data) {
          setSelectedFood(updated.data as FoodWithPortionsDto);
        }
        fetchFoods(data.page);
      } else {
        showFeedback("error", res.error || "Erro ao adicionar porção.");
      }
    } finally {
  ...
  async function handleArchivePortion(portionPublicId: string) {
    if (!selectedFood) return;
    if (!confirm("Deseja arquivar esta medida/porção global?")) return;

    try {
      const res = await archiveGlobalPortionAction(portionPublicId);
      if (res.success) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--border)] pb-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-1">
            Gestão do catálogo oficial de alimentos e porções globais (TACO e manuais).
          </p>
        </div>
        <div>
          <button
      ...
      {/* Filters Bar */}
      <div className="bg-[var(--surface-primary)] border border-[var(--border)] rounded-xl p-4 space-y-3 shadow-sm">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex-1 relative">
            <input
              ...
              placeholder="Buscar por alimento global (ex: arroz, aveia, frango)..."
          ...
          <button
            ...
            disabled={isPending}
            className="px-4 py-2 rounded-lg bg-[var(--surface-secondary)] border border-[var(--border)] text-sm font-medium hover:bg-[var(--surface-tertiary)]"
          >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-[var(--text-muted)] px-1">
          <span>{data.total} alimentos globais</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {data.items.map((food) => (
            <div
              key={food.publicId}
              className="bg-[var(--surface-primary)] border border-[var(--border)] rounded-xl p-4 shadow-sm flex flex-col justify-between gap-3"
            >
              <div className="space-y-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  >
                    {food.status === "ACTIVE" ? "Ativo" : "Arquivado"}
                  </span>
                </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-[var(--border)] text-xs">
                ...
                >
                  Porções ({food.portionsCount})
                </button>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setEditingFood(food)}
                    className="px-2.5 py-1 rounded bg-[var(--surface-secondary)] hover:bg-[var(--surface-tertiary)] font-medium"
                  >
                    Editar
                  </button>
                  {food.status === "ACTIVE" && (
                    <button
                      ...
                      className="px-2 py-1 rounded text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30"
                    >
                      Arquivar
                    </button>
                  )}
              ...
              <>
                <div className="flex items-start justify-between border-b border-[var(--border)] pb-3">
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">
                      {selectedFood.category || "Sem categoria"} • ID Fonte: {selectedFood.sourceUid || "MANUAL"}
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="space-y-3 pt-2">
                  <h3 className="text-sm font-semibold text-[var(--text-primary)]">Porções e Medidas Usuais</h3>
                      ...
                      {selectedFood.portions.map((p) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="text-rose-600 hover:text-rose-700 text-xs px-2 py-0.5 rounded hover:bg-rose-50"
                          >
                            Remover
                          </button>
                        </div>
                ...
                {/* Add portion form for platform admin */}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[var(--surface-primary)] border border-[var(--border)] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
              <h2 className="text-lg font-bold text-[var(--text-primary)]">Novo Alimento Global (Trevo One)</h2>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateFood} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    name="category"
                    type="text"
                    placeholder="Ex: Leite e derivados"
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-[var(--border)]">
                ...
                >
                  {isPending ? "Salvando..." : "Cadastrar Alimento Global"}
                </button>
              </div>
            </form>
          </div>
      ...
      {editingFood && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[var(--surface-primary)] border border-[var(--border)] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
              <h2 className="text-lg font-bold text-[var(--text-primary)]">Editar Alimento Global</h2>
              <button
                type="button"
